...
def send_email_tool(to_email: str, subject: str, body_html: str) -> str:
    """
    Sends an email with the final trip itinerary to the user using a Gmail account via yagmail.
    Args:
    ...
    if not (settings.GMAIL_SENDER_EMAIL and settings.GMAIL_APP_PASSWORD):
        return "Error: Email functionality is disabled because Gmail credentials are not configured in the .env file."
